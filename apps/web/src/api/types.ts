...
export interface RateLimitConfig { emailSent: number; smsSent: number; tokenRefresh: number; tokenVerification: number; anonymousUsers: number; signupsAndSignins: number }
export interface MFAConfig { totpEnrollEnabled: boolean; totpVerifyEnabled: boolean; phoneEnrollEnabled: boolean; phoneVerifyEnabled: boolean; maxEnrolledFactors: number; phoneOtpLength: number }
export interface EmailTemplateConfig { subject: string; templateUrl: string }
export interface EmailNotificationConfig { enabled: boolean; template: EmailTemplateConfig }
export interface MailerConfig { templates: { confirmation: EmailTemplateConfig; invite: EmailTemplateConfig; magicLink: EmailTemplateConfig; emailChange: EmailTemplateConfig; recovery: EmailTemplateConfig; reauthentication: EmailTemplateConfig }; notifications: { passwordChanged: EmailNotificationConfig; emailChanged: EmailNotificationConfig; phoneChanged: EmailNotificationConfig; identityLinked: EmailNotificationConfig; identityUnlinked: EmailNotificationConfig; mfaFactorEnrolled: EmailNotificationConfig; mfaFactorUnenrolled: EmailNotificationConfig } }
export interface AuthConfig { enabled: boolean; jwtExpiry: number; disableSignup: boolean; email: EmailAuthConfig; phone: PhoneAuthConfig; anonymousSignIn: boolean; redirectUrls: string[]; oauth: Record<string, OAuthProviderConfig>; smtp: SMTPConfig; mailer: MailerConfig; rateLimits: RateLimitConfig; mfa: MFAConfig }
...
export interface RedactedAuthConfig { enabled: boolean; jwtExpiry: number; disableSignup: boolean; email: EmailAuthConfig; phone: RedactedPhoneAuthConfig; anonymousSignIn: boolean; redirectUrls?: string[]; oauth?: Record<string, RedactedOAuthProviderConfig>; smtp: RedactedSMTPConfig; mailer: MailerConfig; rateLimits: RateLimitConfig; mfa: MFAConfig }
