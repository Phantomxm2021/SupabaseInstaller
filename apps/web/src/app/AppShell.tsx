              ...
              <SidebarMenu>
                <SidebarMenuItem>
                  <SidebarMenuButton className="primary-sidebar-menu-button" collapsibleIcon={false} isActive={location.pathname === '/projects'} render={<NavLink to="/projects" end />}>
                    <Boxes /> <span>Servers</span>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              </SidebarMenu>
            </nav>}
            {isProjectRoute && projectId && <nav aria-label="Server navigation" className="primary-project-navigation">
              <SidebarMenu>
                ...
                <SidebarMenuItem><SidebarMenuButton className="primary-sidebar-menu-button" collapsibleIcon={false} isActive={location.pathname.includes('/functions')} render={<Link to={`/projects/${projectId}/functions`} aria-current={location.pathname === `/projects/${projectId}/functions` ? 'page' : undefined} />}><Code2 /><span>Functions</span></SidebarMenuButton></SidebarMenuItem>
      ...
      <SidebarInset>
        <header className="topbar" data-density="dashboard" aria-label="Dashboard header">
          ...
          <div className="topbar-left">
            ...
            {isProjectRoute && <><span className="topbar-slash" aria-hidden="true" /><DropdownMenu onOpenChange={(open) => { if (!open) setProjectSearch('') }}><DropdownMenuTrigger className="topbar-project-trigger" aria-label="Show servers"><Database /><span>{activeProject?.name ?? projectId}</span><ChevronsUpDown /></DropdownMenuTrigger><DropdownMenuContent className="topbar-menu topbar-project-menu" sideOffset={8} align="start"><label className="topbar-project-search"><Search /><input aria-label="Find servers" autoFocus value={projectSearch} onChange={(event) => setProjectSearch(event.target.value)} placeholder="Find servers..." /></label>{visibleProjects.map((project) => <DropdownMenuItem key={project.id} render={<Link to={`/projects/${project.id}/overview`} />} className="topbar-menu-item" data-current={project.id === projectId || undefined}><span>{project.name}</span>{project.id === projectId && <span className="topbar-menu-current">✓</span>}</DropdownMenuItem>)}<DropdownMenuSeparator /><DropdownMenuItem render={<Link to="/projects/new" />} className="topbar-menu-item topbar-menu-create"><Plus /><span>New server</span></DropdownMenuItem></DropdownMenuContent></DropdownMenu><span className="topbar-slash" aria-hidden="true" /><DropdownMenu><DropdownMenuTrigger className="topbar-project-trigger topbar-branch-trigger" aria-label="Show branches"><span>main</span><Badge variant="outline">Local</Badge><ChevronsUpDown /></DropdownMenuTrigger><DropdownMenuContent className="topbar-menu topbar-branch-menu" sideOffset={8} align="start"><DropdownMenuRadioGroup value="main"><DropdownMenuRadioItem value="main" className="topbar-menu-item"><span>main</span><Badge variant="outline">Local</Badge></DropdownMenuRadioItem></DropdownMenuRadioGroup></DropdownMenuContent></DropdownMenu></>}
          </div>
        </header>
