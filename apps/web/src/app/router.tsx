...
import {
  ComingSoonPage,
  ProjectLayout,
} from "../features/project/ProjectLayout";
...
function EntryGate() {
  const setup = useQuery({
    ...
    queryFn: () => apiFetch<{ required: boolean }>("/api/setup/status"),
  });
  if (setup.isLoading)
    return <div className="splash">Starting Supabase Manager…</div>;
...
export function AuthenticatedShell() {
  ...
  if (session.isLoading)
    return <div className="splash">Loading workspace…</div>;
      ...
      children: [
        ...
        { path: "/projects/new", element: <NewProjectPage /> },
        {
          ...
          children: [
            ...
            { path: "functions/secrets", element: <FunctionSecretsPage /> },
            {
              path: "authentication",
              element: <AuthenticationWorkspace />,
              children: [
                { index: true, element: <Navigate to="emails" replace /> },
                { path: "users", element: <RetainedAuthenticationRedirect /> },
                {
                  path: "oauth-apps",
                  element: <RetainedAuthenticationRedirect />,
                },
                {
                ...
                { path: "emails", element: <EmailsRoute /> },
                {
                  ...
                  element: <EmailTemplateEditorRoute />,
                },
                { path: "rate-limits", element: <RateLimitsRoute /> },
                { path: "multi-factor", element: <MultiFactorRoute /> },
                {
                  path: "url-configuration",
                  element: <URLConfigurationRoute />,
                },
                ...unsupportedAuthenticationRoutes.map(([path, label]) => ({
                  path,
                  element: <AuthenticationUnavailablePage title={label} />,
                })),
              ],
            },
            { path: "logs", element: <ComingSoonPage /> },
            { path: "backups", element: <ComingSoonPage /> },
            { path: "advisors", element: <ComingSoonPage /> },
            { path: "observability", element: <ComingSoonPage /> },
...
export function NotFoundPage() {
  return (
    <main className="page">
      ...
      <p className="muted">The requested Manager page does not exist.</p>
    </main>
  );
}
