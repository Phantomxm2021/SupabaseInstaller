...
export function SecurityIntegrationsStep({ form }: { form: UseFormReturn<ProjectForm> }) {
  ...
  const setBackend = (backend: string) => {
    ...
    const clean = backend === 'local' ? { ...base, backend, bucket: '', region: '', endpoint: '', accountId: '', accessKeyId: '', secretAccessKeySet: false, secretAccessKey: { action: '' as const }, forcePathStyle: false } : { ...base, backend, region: backend === 'r2' ? '' : base.region, endpoint: backend === 's3' ? base.endpoint : '', accountId: backend === 'r2' ? base.accountId : '', forcePathStyle: backend === 'r2' ? true : base.forcePathStyle }
    set('configuration.storage', clean)
  }
  const [uploadLimitInput, setUploadLimitInput] = useState<number | string>(() => Math.round(storage.uploadFileSizeLimit / (1024 * 1024)))
  const resetUploadLimit = () => { form.setValue('configuration.storage.uploadFileSizeLimit', 50 * 1024 * 1024, { shouldDirty: true, shouldValidate: true }); form.clearErrors('configuration.storage.uploadFileSizeLimit' as any); setUploadLimitInput(50) }
  useEffect(() => { if (!services.storage || storage.backend === 'local') resetUploadLimit() }, [services.storage, storage.backend])
  ...
  return <div className="space-y-4">
    <Module title="Authentication" enabled={services.auth} onEnabledChange={(enabled) => setServiceEnabled(form, 'auth', enabled)} headerAction={services.auth && <AuthMethodDialog open={dialogOpen} onOpenChange={setDialogOpen} onSelect={selectMethod} addedOAuth={addedOAuth} />}>
      <TextField label="Auth site URL" name="configuration.general.authSiteUrl" form={form} />
