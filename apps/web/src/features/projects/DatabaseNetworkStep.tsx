...
export function GatewayNetworkFields({ form }: InfrastructureFormProps) {
  ...
  const set = (name: string, value: unknown) => { form.setValue(name as never, value as never, { shouldDirty: true, shouldValidate: true }); form.setValue('preset', 'CUSTOM', { shouldDirty: true }) }
  return <section className="space-y-4"><FieldGroup className="grid gap-4 md:grid-cols-2"><Field><FieldLabel>Gateway</FieldLabel><Select value={network.gateway} onValueChange={(value) => set('configuration.network.gateway', value)}><SelectTrigger aria-label="Gateway"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="envoy">Envoy</SelectItem><SelectItem value="kong">Kong (advanced)</SelectItem></SelectContent></Select></Field><Field><FieldLabel>HTTPS mode</FieldLabel><Select value={network.httpsMode} onValueChange={(value) => { if (value === 'caddy') setServiceEnabled(form, 'gateway', true); set('configuration.network.httpsMode', value) }}><SelectTrigger aria-label="HTTPS mode"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="external">External reverse proxy</SelectItem><SelectItem value="caddy">Caddy managed</SelectItem></SelectContent></Select><FieldDescription>Manual certificates are not supported by this renderer and cannot be submitted. Caddy automatically enables the API Gateway dependency.</FieldDescription></Field><ReadOnlyPort label="Internal gateway port" value={network.internalGatewayPort} /><ReadOnlyPort label="API port" value={network.apiPort} /><ReadOnlyPort label="Studio port" value={network.studioPort} /><ReadOnlyPort label="Direct database allocated port" value={network.directDatabasePort} /><ReadOnlyPort label="Pooler allocated port" value={network.poolerPort} /></FieldGroup>{network.httpsMode === 'external' && <Alert>External reverse proxy mode emits upstream instructions; TLS is terminated outside this manager.</Alert>}</section>
}
