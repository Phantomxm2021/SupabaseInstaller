...
export function NewProjectPage() {
  const [step, setStep] = useState(0); const [direction, setDirection] = useState<WizardStepDirection>('forward'); const [operation, setOperation] = useState<{ projectId: string; operationId: string }>(); const [openedInfrastructureSection, setOpenedInfrastructureSection] = useState<InfrastructureSectionId>(); const [tls, setTLS] = useState<{ enabled: boolean; certificate?: File; privateKey?: File }>({ enabled: false }); const navigate = useNavigate()
  ...
  const create = useMutation({ mutationFn: (value: ProjectForm) => { const configuration = normalizeCreateConfiguration(value.configuration); const body: CreateProjectRequest = { name: value.name, slug: value.slug, preset: value.preset, configuration }; if (tls.enabled) { if (!tls.certificate || !tls.privateKey) throw new Error('Upload both the certificate and its matching private key'); const formData = new FormData(); formData.set('draft', JSON.stringify(body)); formData.set('certificate', tls.certificate); formData.set('privateKey', tls.privateKey); return apiFetch<{ projectId: string; operationId: string }>('/api/projects', { method: 'POST', body: formData }) } return apiFetch<{ projectId: string; operationId: string }>('/api/projects', { method: 'POST', body: JSON.stringify(body) }) }, onSuccess: setOperation, onError: (error) => { if (error instanceof APIError && error.status === 409) { form.setError('root.project', { message: error.message }); queryClient.invalidateQueries({ queryKey: ['projects'] }); moveToStep(0) } } })
  ...
  const install = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (tls.enabled && (!tls.certificate || !tls.privateKey)) { form.setError('root.project', { message: 'Upload both the certificate and its matching private key' }); moveToStep(0); return }
  ...
  return <main className="page narrow-page"><div className="page-heading" ref={wizardHeader}><PageHeader eyebrow="New runtime" title={step === wizardSteps.length - 1 ? 'Review installation' : 'Create a server'} description="Configure the complete Supabase runtime before Docker resources are created." actions={<span className="wizard-step">Step {step + 1} of {wizardSteps.length} · {currentStep.label}</span>} /></div><Form {...form}><form className="wizard mt-3" onSubmit={install} noValidate><WizardStepFrame key={currentStep.id} step={currentStep.id} direction={direction}>{step === 0 && <BasicStep form={form} availability={availability} onRetryAvailability={() => { void projects.refetch() }} tls={tls} onTLSChange={setTLS} />}{step === 1 && <ServiceConfiguration form={form} />}{step === 2 && <SecurityIntegrationsStep form={form} />}{step === 3 && <div className="space-y-6"><InfrastructureReviewStep form={form} openedSection={openedInfrastructureSection} /><ReviewStep project={values} onEdit={(target) => moveToStep(wizardSteps.findIndex((item) => item.id === target))} onEditInfrastructure={() => revealInfrastructureSection('database-realtime', true)} /></div>}</WizardStepFrame>{(form.formState.errors.root || create.error) && <Alert variant="destructive" className="mt-4">{String(form.formState.errors.root?.message || create.error?.message)}</Alert>}<div className="wizard-footer"><Button variant="secondary" nativeButton={false} render={<Link to="/projects" />}><ArrowLeft size={15} />Cancel</Button><div className="flex gap-2">{step > 0 && <Button variant="secondary" type="button" onClick={() => moveToStep(step - 1)}><ArrowLeft size={15} />Back</Button>}{step < wizardSteps.length - 1 && <Button type="button" disabled={step ===0 && !firstStepCanContinue} onClick={next}>Continue<ArrowRight size={15} /></Button>}{step === wizardSteps.length - 1 && <Button type="submit" disabled={create.isPending} aria-busy={create.isPending}>{create.isPending ? <Spinner data-icon="inline-start" aria-label="Creating operation" /> : <Rocket size={15} />}{create.isPending ? 'Creating operation…' : 'Install server'}</Button>}</div></div></form></Form></main>
}
