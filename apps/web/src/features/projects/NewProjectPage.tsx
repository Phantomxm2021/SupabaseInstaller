...
export function NewProjectPage() {
  ...
  const create = useMutation({ mutationFn: (value: ProjectForm) => { const configuration = normalizeCreateConfiguration(value.configuration); const body: CreateProjectRequest = { name: value.name, slug: value.slug, supabaseVersion: configuration.general.supabaseVersion, preset: value.preset, configuration }; return apiFetch<{ projectId: string; operationId: string }>('/api/projects', { method: 'POST', body: JSON.stringify(body) }) }, onSuccess: setOperation })
