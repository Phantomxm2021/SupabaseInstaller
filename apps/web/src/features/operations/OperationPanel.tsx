...
import { AlertTriangle, Check, LoaderCircle, RotateCcw, ShieldAlert, Trash2 } from 'lucide-react'
...
const labels: Record<string, string> = {
  ...
  MARK_RUNNING: 'Mark server running',
...
function OperationPanelCore({ operationId, projectId, projectName, onSucceeded, onDeleted, navigate }: Props & { navigate?: (to: string, options?: { replace?: boolean }) => void }) {
  ...
  const action = useMutation<{ operationId: string } | void, Error, RecoveryAction>({
    mutationFn: (name) => {
      const id = operation.data?.projectId || projectId
      if (name === 'delete') return apiFetch<void>(`/api/projects/${id}`, { method: 'DELETE', body: JSON.stringify({ mode: 'data', confirmation: projectName }) })
      return apiFetch<{ operationId: string }>(`/api/projects/${id}/${name}`, { method: 'POST' })
    },
    onSuccess: async (result, name) => {
      if (name === 'delete') {
        await queryClient.invalidateQueries({ queryKey: ['projects'] })
        onDeleted?.()
        if (!onDeleted) navigate?.('/projects', { replace: true })
        return
      }
      if (!result || typeof result !== 'object' || !('operationId' in result)) return
      setActiveOperationId(result.operationId)
      void queryClient.invalidateQueries({ queryKey: ['operation', result.operationId] })
    },
  })
