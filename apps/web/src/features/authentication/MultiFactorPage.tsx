...
export function MultiFactorPage({ context: provided }: { context?: AuthenticationWorkspaceContext }) {
  ...
  return <main className="page auth-page space-y-16">
    ...
    <form className="space-y-12" onSubmit={form.handleSubmit((value) => context.requestSave({ section: 'auth', value, dirty: form.formState.dirtyFields, setError: (name, message) => form.setError(name as never, { type: 'server', message }) }))}>
      <MFASection title="Multi-Factor Authentication (MFA)">
        <Toggle className="auth-setting-toggle auth-mfa-toggle" id="totp-enrollment" label="TOTP (App Authenticator) enrollment" description="Allow users to enroll an authenticator application as an additional factor." checked={mfa.totpEnrollEnabled} onChange={(value) => set('totpEnrollEnabled', value)} />
        <Toggle className="auth-setting-toggle auth-mfa-toggle" id="totp-verification" label="TOTP (App Authenticator) verification" description="Allow enrolled authenticator factors to verify sign-ins." checked={mfa.totpVerifyEnabled} onChange={(value) => set('totpVerifyEnabled', value)} />
      ...
      <MFASection title="SMS MFA">
        <Toggle className="auth-setting-toggle auth-mfa-toggle" id="phone-mfa-enrollment" label="Phone MFA enrollment" description="Allow users to enroll a phone number as a multi-factor authentication method." checked={mfa.phoneEnrollEnabled} onChange={(value) => set('phoneEnrollEnabled', value)} />
        <Toggle className="auth-setting-toggle auth-mfa-toggle" id="phone-mfa-verification" label="Phone MFA verification" description="Allow enrolled phone factors to verify sign-ins with an SMS code." checked={mfa.phoneVerifyEnabled} onChange={(value) => set('phoneVerifyEnabled', value)} />
...
function MFASection({ title, children }: { title: string; children: React.ReactNode }) { return <section className="space-y-4"><h2>{title}</h2><div className="auth-settings-card auth-mfa-card">{children}</div></section> }
