...
function EmailForm({ initial, onDirty, onSave, onQueued }: { initial: AuthConfig; onDirty: (value: boolean) => void; onSave: Save; onQueued: () => void }) {
  ...
  return <form className="space-y-4 p-4" onSubmit={form.handleSubmit((value) => onSave({ section: 'auth', value, dirty: form.formState.dirtyFields, onQueued }))}><Toggle id="email-auth" label="Email Auth" checked={email.enabled} onChange={(value) => form.setValue('email.enabled', value, { shouldDirty: true, shouldValidate: true })} /><Toggle id="secure-email-change" label="Secure Email Change" checked={email.secureEmailChange} onChange={(value) => form.setValue('email.secureEmailChange', value, { shouldDirty: true, shouldValidate: true })} /><Toggle id="double-confirm-email-change" label="Double confirmation for email changes" checked={email.doubleConfirmChanges} onChange={(value) => form.setValue('email.doubleConfirmChanges', value, { shouldDirty: true, shouldValidate: true })} /><UnavailableSecurity /><SaveButton disabled={!form.formState.isDirty} /></form>
}
