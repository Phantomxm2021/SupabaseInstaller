...
function EmailForm({ initial, onDirty, onSave, onQueued }: { initial: AuthConfig; onDirty: (value: boolean) => void; onSave: Save; onQueued: () => void }) {
  ...
  return <form className="space-y-4 p-4" onSubmit={form.handleSubmit((value) => onSave({ section: 'auth', value, dirty: form.formState.dirtyFields, onQueued }))}><Toggle id="email-auth" label="Email Auth" checked={email.enabled} onChange={(value) => form.setValue('email.enabled', value, { shouldDirty: true, shouldValidate: true })} /><Toggle id="secure-email-change" label="Secure Email Change" checked={email.secureEmailChange} onChange={(value) => { form.setValue('email.secureEmailChange', value, { shouldDirty: true, shouldValidate: true }); form.setValue('email.doubleConfirmChanges', value, { shouldDirty: true, shouldValidate: true }) }} /><UnavailableSecurity /><SaveButton disabled={!form.formState.isDirty} /></form>
}
...
function UnavailableSecurity() { return <div className="rounded-lg border border-border p-3"><p className="text-sm font-medium">Security options</p><p className="mt-1 text-xs text-muted-foreground">Password strength and CAPTCHA controls are unavailable in this Manager version.</p></div> }
