...
function EmailForm({ initial, onDirty, onSave, onQueued }: { initial: AuthConfig; onDirty: (value: boolean) => void; onSave: Save; onQueued: () => void }) {
  ...
  return <form className="authentication-provider-form space-y-0" onSubmit={form.handleSubmit((value) => onSave({ section: 'auth', value, dirty: form.formState.dirtyFields, onQueued }))}><Toggle className="auth-setting-toggle auth-drawer-setting" id="email-auth" label="Enable email provider" description="Allow email-based sign up and sign in for your application." checked={email.enabled} onChange={(value) => form.setValue('email.enabled', value, { shouldDirty: true, shouldValidate: true })} /><Toggle className="auth-setting-toggle auth-drawer-setting" id="secure-email-change" label="Secure email change" description="Require confirmation on both the current and new email address before applying a change." checked={email.secureEmailChange} onChange={(value) => { form.setValue('email.secureEmailChange', value, { shouldDirty: true, shouldValidate: true }); form.setValue('email.doubleConfirmChanges', value, { shouldDirty: true, shouldValidate: true }) }} /><SaveButton disabled={!form.formState.isDirty} /></form>
}
