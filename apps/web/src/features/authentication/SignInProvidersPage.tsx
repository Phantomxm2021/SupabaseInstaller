      ...
      <TabsContent value="supabase" className="dashboard-stack">
        <form className="dashboard-section" onSubmit={form.handleSubmit((value) => requestSave({ section: 'auth', value, dirty: form.formState.dirtyFields, setError: (name, message) => form.setError(name as never, { type: 'server', message }) }))}>
          ...
          <section className="auth-settings-card">
            ...
            <SettingsToggleRow><Toggle className="auth-setting-toggle" id="manual-linking" label="Allow manual linking" description="Enable manual linking APIs for your server." checked={Boolean(policy.manualLinking)} onChange={(value) => form.setValue('manualLinking', value, { shouldDirty: true, shouldValidate: true })} /></SettingsToggleRow>
