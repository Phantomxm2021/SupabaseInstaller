...
import { SectionCard, NumberField, TextField, ReadOnlyField, SectionSaveButton, useResetOnServerRevision } from './fields'
export function DatabaseSection({ initial, revision, directDb, onSave, onRotate }: { initial: DatabaseConfig; revision: number; directDb: boolean; onSave: (value: DatabaseConfig, dirty: unknown) => void; onRotate: () => void }) {
  const form = useForm<DatabaseConfig>({ resolver: zodResolver(databaseSchema) as Resolver<DatabaseConfig>, defaultValues: initial }); useResetOnServerRevision(form, initial, revision); const db = form.watch()
