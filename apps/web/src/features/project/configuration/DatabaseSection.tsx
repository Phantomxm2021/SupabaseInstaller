...
export function DatabaseSection({ initial, revision, directDb, onSave, onRotate }: { initial: DatabaseConfig; revision: number; directDb: boolean; onSave: SectionSave<DatabaseConfig>; onRotate: () => void }) {
  ...
  return <form id="configuration-database-form" onSubmit={form.handleSubmit((value) => onSave({ value, dirty: form.formState.dirtyFields, setError }))} className="space-y-5"><SectionCard title="Database" description="Postgres version and limits. Database service toggles are owned by Services."><div className="space-y-4"><ReadOnlyField label="Postgres version" value={db.version} /><p className="rounded-lg border border-border p-3 text-sm text-muted-foreground">Direct PostgreSQL port is currently <strong>{directDb ? 'enabled' : 'disabled'}.</strong></p><NumberField form={form} name="maxConnections" label="Maximum connections" min={1} max={100000} /><TextField form={form} name="sharedBuffers" label="Shared buffers" /><p className="rounded-lg border border-amber-500/40 bg-amber-500/5 p-3 text-sm text-muted-foreground">Saving database settings recreates PostgreSQL and briefly restarts dependent services.</p></div><ReadOnlyField label="Supported extensions" value={db.extensions.join(', ') || 'None supported by pinned runtime'} /><Button type="button" variant="destructive" onClick={onRotate}>Rotate database password</Button></SectionCard><SectionSaveButton label="Database" disabled={!form.formState.isDirty} /></form>
}
