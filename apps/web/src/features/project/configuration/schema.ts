...
export const authSchema = z.object({ enabled: z.boolean(), jwtExpiry: z.number().int().min(0).max(31536000), disableSignup: z.boolean(), email: emailAuthSchema, phone: phoneUpdateSchema, anonymousSignIn: z.boolean(), redirectUrls: z.array(httpURL), oauth: z.record(z.string(), providerUpdateSchema), smtp: smtpUpdateSchema }).superRefine((value, context) => {
