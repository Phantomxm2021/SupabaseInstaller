...
export function ServicesSection({ initial, onSave }: { initial: Services; onSave: (value: Services, dirty: unknown) => void }) {
  ...
  const change = (name: keyof Services, value: boolean) => { const next = { ...form.getValues(), [name]: value }; if (name === 'studio') next.postgresMeta = value; if (name === 'postgresMeta' && !value) next.studio = false; if (name === 'logs') next.vector = value; if (name === 'storage' && !value) next.imgproxy = false; if (name === 'imgproxy' && value) next.storage = true; if (name === 'gateway' && !value) { next.auth = false; next.rest = false; next.studio = false; next.postgresMeta = false; next.realtime = false; next.storage = false; next.imgproxy = false; next.functions = false } form.reset(next, { keepDirty: true }); for (const key of Object.keys(next) as Array<keyof Services>) if (next[key] !== initial[key]) form.setValue(key, next[key], { shouldDirty: true, shouldValidate: true }) }
