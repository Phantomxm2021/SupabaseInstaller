...
import { SectionCard, NumberField, ReadOnlyField, SectionSaveButton, useResetOnServerRevision } from './fields'
export function PoolerSection({ initial, revision, enabled, onSave }: { initial: PoolerConfig; revision: number; enabled: boolean; onSave: (value: PoolerConfig, dirty: unknown) => void }) {
  const form = useForm<PoolerConfig>({ resolver: zodResolver(poolerSchema) as Resolver<PoolerConfig>, defaultValues: initial }); useResetOnServerRevision(form, initial, revision); const pooler = form.watch()
