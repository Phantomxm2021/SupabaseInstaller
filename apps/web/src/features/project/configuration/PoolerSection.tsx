...
export function PoolerSection({ initial, revision, enabled, onSave }: { initial: PoolerConfig; revision: number; enabled: boolean; onSave: SectionSave<PoolerConfig> }) {
  ...
  return <form id="configuration-pooler-form" onSubmit={form.handleSubmit((value) => onSave({ value, dirty: form.formState.dirtyFields, setError }))} className="space-y-5"><SectionCard title="Connection Pooler" description="Supavisor enablement is owned by Services. Ports are allocated by Manager."><p className="text-sm text-muted-foreground">Supavisor is currently <strong>{enabled ? 'enabled' : 'disabled'}.</strong></p><div className="space-y-4"><ReadOnlyField label="Transaction port" value={String(pooler.transactionPort || 'Pending allocation')} /><ReadOnlyField label="Session port" value={String(pooler.sessionPort || 'Pending allocation')} /><NumberField form={form} name="poolSize" label="Pool size" min={1} max={100000} /><NumberField form={form} name="internalDbPoolSize" label="Internal database pool size" min={1} max={100000} /><NumberField form={form} name="maxClientConnections" label="Maximum client connections" min={1} max={100000} /></div></SectionCard><SectionSaveButton label="Connection Pooler" disabled={!form.formState.isDirty} /></form>
}
