...
function ProviderForm({ provider, initial, callback, onSave }: { provider: string; initial: OAuthProviderConfig; callback: string; onSave: (provider: string, value: OAuthProviderConfig, dirty: unknown) => void }) {
  ...
  return <form id={`configuration-oauth-${provider}-form`} onSubmit={form.handleSubmit((next) => onSave(provider, next, form.formState.dirtyFields))} className="space-y-3 rounded-lg border border-border p-4"><div className="flex items-center justify-between gap-2"><h3 className="font-medium">{labels[provider] ?? provider}</h3><Toggle id={`oauth-${provider}-enabled`} label={`Enable ${labels[provider] ?? provider}`} checked={value.enabled} onChange={(enabled) => form.setValue('enabled', enabled, { shouldDirty: true, shouldValidate: true })} /></div>{value.enabled && <><TextField form={form} name="clientId" label="Client ID" /><SecretEditor label="Client secret" configured={value.secretSet} secret={value.secret} onChange={(secret) => form.setValue('secret', secret, { shouldDirty: true, shouldValidate: true })} />{special && <TextField form={form} name={`fields.${special}` as 'fields'} label={special} />}<ReadOnlyField label="Callback URL" value={callback} copy={callback.startsWith('http')} /><button type="submit" className="w-full rounded-md border px-3 py-2 text-sm" disabled={!form.formState.isDirty}>Save {labels[provider] ?? provider}</button></>}</form>
}
