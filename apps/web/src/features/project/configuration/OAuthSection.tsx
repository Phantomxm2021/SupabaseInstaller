...
import { SectionCard, TextField, SecretEditor, Toggle, ReadOnlyField } from './fields'
...
export function OAuthSection({ initial, general, onSave }: { initial: Record<string, OAuthProviderConfig>; general: GeneralConfig; onSave: (provider: string, value: OAuthProviderConfig, dirty: unknown) => void }) {
  const callback = general.domain ? `https://${general.domain}/auth/v1/callback` : 'Set Domain to generate callback'
  return <SectionCard title="OAuth providers" description="Each provider is saved independently. Callback URLs are generated from the project public URL and never contain query parameters."><div className="grid gap-4 md:grid-cols-2">{OAUTH_PROVIDERS.map((provider) => <ProviderForm key={provider} provider={provider} initial={initial[provider] ?? emptyProvider()} callback={callback} onSave={onSave} />)}</div></SectionCard>
}
function ProviderForm({ provider, initial, callback, onSave }: { provider: string; initial: OAuthProviderConfig; callback: string; onSave: (provider: string, value: OAuthProviderConfig, dirty: unknown) => void }) {
  const form = useForm<OAuthProviderConfig>({ resolver: zodResolver(oauthProviderSchema(provider)) as Resolver<OAuthProviderConfig>, defaultValues: initial }); useEffect(() => { form.reset(initial) }, [initial, form]); const value = form.watch(); const special = specialFields[provider]
  return <form id={`configuration-oauth-${provider}-form`} onSubmit={form.handleSubmit((next) => onSave(provider, next, form.formState.dirtyFields))} className="space-y-3 rounded-lg border border-border p-4"><div className="flex items-center justify-between gap-2"><h3 className="font-medium">{labels[provider] ?? provider}</h3><Toggle id={`oauth-${provider}-enabled`} label={`Enable ${labels[provider] ?? provider}`} checked={value.enabled} onChange={(enabled) => form.setValue('enabled', enabled, { shouldDirty: true, shouldValidate: true })} /></div><TextField form={form} name="clientId" label="Client ID" /> <SecretEditor label={`${labels[provider] ?? provider} client secret`} configured={value.secretSet} secret={value.secret} onChange={(secret) => form.setValue('secret', secret, { shouldDirty: true, shouldValidate: true })} />{special && <TextField form={form} name={`fields.${special}` as 'fields'} label={special} />}{value.enabled && <ReadOnlyField label="Callback URL" value={callback} copy={callback.startsWith('http')} />}<button type="submit" className="w-full rounded-md border px-3 py-2 text-sm" disabled={!form.formState.isDirty}>Save {labels[provider] ?? provider}</button></form>
}
