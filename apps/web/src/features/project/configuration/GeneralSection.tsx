...
export function GeneralSection({ initial, revision, onSave, serverErrors = {} }: Props) {
  ...
  return <form id="configuration-general-form" onSubmit={form.handleSubmit((value) => onSave({ value, dirty: form.formState.dirtyFields, setError: (name, message) => form.setError(name as 'siteUrl', { type: 'server', message }) }))} className="space-y-5"><SectionCard title="General" description="The public project hostname is generated from the slug and Site URL base domain."><div className="grid gap-4 md:grid-cols-2"><TextField form={form} name="siteUrl" label="Site URL base domain" placeholder="https://platform.example.com" /><ReadOnlyField label="Domain" value={domain || 'Save a base domain to generate Domain'} /><ReadOnlyField label="Supabase version" value={initial.supabaseVersion} /><ReadOnlyField label="Project URL" value={domain ? `https://${domain}` : 'Save a base domain to generate Project URL'} copy={Boolean(domain)} /><ReadOnlyField label="OAuth callback URL" value={callback} copy={Boolean(domain)} /></div></SectionCard><SectionSaveButton label="General" disabled={!form.formState.isDirty} /></form>
}
