...
export function GeneralSection({ initial, revision, onSave, serverErrors = {} }: Props) {
  ...
  return <form id="configuration-general-form" onSubmit={form.handleSubmit((value) => onSave({ value, dirty: form.formState.dirtyFields, setError: (name, message) => form.setError(name as 'siteUrl', { type: 'server', message }) }))} className="space-y-5"><SectionCard title="General" description="The public server hostname is generated from the slug and Site URL base domain."><div className="space-y-4"><TextField form={form} name="siteUrl" label="Site URL base domain" placeholder="https://platform.example.com" /><ReadOnlyField label="Public server hostname" value={domain || 'Save a base domain to generate a public server hostname'} /><ReadOnlyField label="Supabase version" value={initial.supabaseVersion} /><ReadOnlyField label="Server URL" value={domain ? `https://${domain}` : 'Save a base domain to generate Server URL'} copy={Boolean(domain)} /><ReadOnlyField label="OAuth callback URL" value={callback} copy={Boolean(domain)} /></div></SectionCard><SectionSaveButton label="General" disabled={!form.formState.isDirty} /></form>
}
