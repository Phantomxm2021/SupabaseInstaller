...
it('uses the Supabase-style overview hierarchy while keeping local project data', async () => {
  ...
  expect(within(hero).queryByRole('button', { name: 'Update Supabase runtime' })).not.toBeInTheDocument()
...
it('requires confirmation before it queues an official runtime sync', async () => {
  ...
  await user.click(await screen.findByRole('menuitem', { name: 'Update Supabase runtime' }))
