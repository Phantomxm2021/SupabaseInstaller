...
func TestOfficialRuntimeSyncQueuesAConfigurationReconcile(t *testing.T) {
	database, err := store.Open(filepath.Join(t.TempDir(), "manager.db"))
	if err != nil {
		t.Fatal(err)
	}
	...
	mux := http.NewServeMux()
	RegisterConfigurationRoutes(mux, ConfigurationOptions{Orchestrator: configuration.NewOrchestrator(database, operation.NewService(database, func() string { return "sync-op" }, time.Now))})

	response := httptest.NewRecorder()
	mux.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/api/projects/bee/runtime/sync", nil))
	if response.Code != http.StatusAccepted || !strings.Contains(response.Body.String(), `"operationId":"sync-op"`) {
