...
// RepairDatabase makes the internal roles and metadata database idempotently
// usable after an interrupted or legacy bootstrap. Postgres only runs
// docker-entrypoint-initdb.d scripts on an empty data directory, while the
// Manager intentionally preserves that directory across a failed install.
// The repair copies the existing postgres role's verifier (rather than
// exposing the configured password in argv) to the service roles and creates
// the internal _supabase database when it is missing.
func (r *Runner) RepairDatabase(ctx context.Context, project ProjectRef) error {
	// The pinned Supabase image protects its internal roles with supautils. The
	// postgres role is intentionally not a superuser there, so use the
	// superuser supabase_admin and pass its password through the db container's
	// environment rather than exposing it in the process argv.
	args := append(r.baseArgs(project), "exec", "-T", "db", "sh", "-lc", `PGPASSWORD="$POSTGRES_PASSWORD" exec psql -v ON_ERROR_STOP=1 -U supabase_admin -d postgres`)
	if inputRunner, ok := r.executor.(InputExecutor); ok {
		statement := `DO $$
DECLARE
  postgres_verifier text;
  role_name text;
BEGIN
  SELECT rolpassword INTO postgres_verifier
    FROM pg_authid
   WHERE rolname = 'postgres';
  IF postgres_verifier IS NULL THEN
    RAISE EXCEPTION 'postgres role has no password verifier';
  END IF;
  ...
  FOREACH role_name IN ARRAY ARRAY[
    'supabase_admin', 'authenticator', 'pgbouncer',
    'supabase_auth_admin', 'supabase_functions_admin',
    'supabase_storage_admin'
  ] LOOP
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = role_name) THEN
      EXECUTE format('ALTER ROLE %I PASSWORD %L', role_name, postgres_verifier);
    END IF;
  END LOOP;
END
$$;
...
`
		if output, err := inputRunner.RunInput(ctx, "docker", args, nil, []byte(statement)); err != nil {
			...
			if detail != "" {
				return fmt.Errorf("database bootstrap repair failed: %w; output=%s", err, detail)
			}
			return fmt.Errorf("database bootstrap repair failed: %w", err)
		}
		return nil
	}
	return errors.New("secure database bootstrap input is unavailable")
}
