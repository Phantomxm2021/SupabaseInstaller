...
host Agent for per-server Nginx sites. Run the installer from the repository
...
- Cloudflare DNS records that point the Manager and server hostnames to this server
...
For non-default server storage, add `--project-root /opt/supabase-manager/projects`.
...
server data, or unrelated Nginx sites.

Open the configured `PUBLIC_ORIGIN`. The first visit creates the administrator
and displays one-time recovery codes.

For public server hosts, configure the Manager hostname separately from each
server's `General.Domain`. The installed native Agent creates and removes only
safe per-server `sites-available`/`sites-enabled` files; `PUBLIC_ORIGIN` alone
does not publish server runtimes. See [Server Supabase host behind Nginx and
Cloudflare](project-host-nginx.md).

To run the disposable HTTP acceptance against an isolated Compose environment,
...
isolation, and cleans up only its own Compose environment.
...
The last command stops the control plane without deleting Manager data or any Supabase server data. Do not add `-v` unless deleting the Manager database is intentional.
